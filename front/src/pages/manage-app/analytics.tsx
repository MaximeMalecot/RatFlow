...
export default function Analytics() {
  ...
  useEffect(() => {
    fetchAnalytics(filters, initialPagination, true);
  }, [app]);

  return (
    <div className="w-full h-fit flex flex-col gap-3" style={{ width: "100%" }}>
      ...
      >
        <AnalyticsFiltersSelector
          ...
          onSubmit={(data) => fetchAnalytics(data, initialPagination, true)}
        />
        <div className="divider my-0"></div>
        <div>
          ...
          {pagination.canLoadMore && (
            <button className="btn w-fit" onClick={fetchMoreAnalytics}>
              More
            </button>
          )}
        </div>
