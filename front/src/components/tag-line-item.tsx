...
import analyticsService, { ClickRate } from "../services/analytics.service";
...
export default function TagLineItem({
    tag,
    copyToClipboard,
    lastCopied,
    deleteTag,
    ...
    const [clickThroughRate, setClickThroughRate] = useState<ClickRate>({
        value: 0,
        unit: "%",
    });
    ...
    const fetchRate = async () => {
        try {
            const rate = await analyticsService.getTagClickThroughRate(
                tag.appId,
                tag._id
            );
    ...
    return (
        <tr>
            <th className="uppercase">{tag.name}</th>
            <td className="flex items-center gap-2">
                ...
                >
                    <path
                        ...
                        fill={lastCopied === tag._id ? "#C2C2C2" : "#3E3E3E"}
                    />
                </svg>
            </td>
            <td>
                {loading
                    ? "..."
                    : `${clickThroughRate.value} ${clickThroughRate.unit}`}
            </td>
